...
function vgDelete(vg_delete) {
	$(vg_delete).on('click', function(e) {
		if(!confirm('Подтвердить удаление')) {
			return false;
		} else if($(this).is(".vg-dotted-square img.vg_delete") || $(this).is(".thumb_img img.vg_delete")) {
			e.stopPropagation();
...
function delGalleryImg(img) {
	...
	let file_name = $(img).closest(".img_wrapper").find("input[type=file]").attr("name");
	...
	if(galImg.img_src && galImg.row) {
		$.ajax({
			...
			success: function(res) {
				if(res) {
					if(galImg.row.indexOf('gallery') != -1){
                        $(img).parent().remove();
					}else{
                        $(img).remove();
					}
...
$(document).ready(function($) {
	...
	$('.single_img, .gallery_img').on('change', function() {
		...
		if($(this).is(".single_img")) {
			...
			if($(this).closest(".img_container").find(".thumbnails_show").length) thumb_flag = true;
		} else {
	...
	/*Згрузка изображений на сервер*/
	function sendAjaxImage(send_image){
		let template = $(send_image).closest("form").attr('action');
		if(template.indexOf('edit') != -1){
			let id = $(send_image).closest("form").find('input[type=hidden]:eq(0)').val();
            let id_row = $(send_image).closest("form").find('input[type=hidden]:eq(0)').attr('name');
            let table = $(send_image).closest("form").find('input[name=table]').val();
            let img_row = $(send_image).attr('name');

			imgSend = new FormData();
			imgSend.append(img_row, send_image.files[0]);
			imgSend.append('id', id);
            imgSend.append('id_row', id_row);
            imgSend.append('table', table);
            imgSend.append('img_row', img_row);
            imgSend.append('ajax', 'upload_image');
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    console.log(res);
                    $(send_image).parent().parent().next().find('img').attr('src', res);
                    $(send_image).closest(".img_container").find(".thumbnails_show").find('img').remove();
                }
            });
		}
	...
	if($('div').is('.sort_panel')){
		var sort_panel = $('.sort_panel').text();
		$(sort_panel).sortable({
			cancel: ".gallery_container",
			handle: '.vg-header',
			stop: function(event, ui){
				var elem_tag = $(ui.item[0]).find('[name]').attr('name');
                var res_elements = {};
                var all_elements = {};
                var i = 0;
				$(this).closest('form').find('[name]').each(function(){
					if($(this).is('input[type=hidden]')) return;
					all_elements[i] = $(this).attr('name');
					if(elem_tag == all_elements[i]){
						if(!i){
                            res_elements['first'] = true;
							res_elements['current'] = elem_tag;
						}else{
							res_elements['current'] = elem_tag;
                            res_elements['after'] = all_elements[i - 1];
						}
						res_elements['table'] = $(this).closest('form').find('input[name=table]').val();
						return false;
					}
                    i++;
				});

				if(res_elements.table != 'undefined'){
                    if(res_elements['current'].lastIndexOf("[]") !== -1) {
                        res_elements['current'] = res_elements['current'].substr(0, res_elements['current'].lastIndexOf("[]"));
                    }
                    ...
                    $.ajax({
                        ...
                        data: {
                            ajax: 'sort_table',
                            data: JSON.stringify(res_elements)
                        },
                        success: function(res) {
                            if(res) {
                                if(res != 'true') {
                                    alert('Ошибка сортировки полей');
                                }
                            } else {
                                alert('Ошибка сортировки полей');
                            }
	...
	if($('select[name=parent_id]').length > 0 && $('select[name=menu_pos]').length > 0) {
		var default_select = $('select[name=parent_id]').val();
		var default_pos = $('select[name=menu_pos]').html();

		$('select[name=parent_id]').on('change', function() {
			if($(this).val() === default_select) {
				$('select[name=menu_pos]').html(default_pos);
			} else {
				var menuPos = {};
				menuPos['table'] = $(this).closest("form").children("input[name=table]").val();
				menuPos['parent_id'] = $(this).val();
				...
				$.ajax({
					...
					data: {
						ajax: 'change_parent',
						data: JSON.stringify(menuPos)
					},
					success: function(res) {
						if(res) {
							res = parseInt(res);
							$('select[name=menu_pos]').empty();
							if(res) {
								for(var i = 1; i <= res; i++) {
									$('select[name=menu_pos]').append('<option value="' + i + '">' + i + '</option>');
								}
							} else {
								$('select[name=menu_pos]').append('<option value="1">1</option>');
							}
						} else {
							alert('Ошибка сортировки изображений');
						}
